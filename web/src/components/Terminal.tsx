...
interface TerminalProps {
  ...
  currentKey?: string
  // Shared header context/actions. Single view and each tiled pane both mount
  // Terminal, so the header lives here and both views get it.
  displayName?: string
  cwd?: string
...
export function Terminal({ sessionName, hostId, backend, fullscreen, onToggleFullscreen, keyBarEnabled = true, onOpenFile, composeTarget, currentKey, displayName, cwd, onSplit, onClose, onKill, onPopOut, draggableHeader, onHeaderDragStart }: TerminalProps) {
  ...
  return (
    <div className="group flex-1 overflow-hidden flex flex-row bg-canvas">
      <div className="h-full w-full flex flex-col p-[3px]">
        {!showMobileKeyBar && (
          <TerminalHeader
            ...
            artifactsCount={serverArtifacts.length}
            onCompose={() => { setClipboardMenuOpen(false); setComposeText(''); setComposeOpen(true) }}
            onToggleArtifacts={() => setArtifactsOpen((v) => !v)}
            onSplit={onSplit}
            onPopOut={onPopOut ?? handlePopOut}
      ...
      {composeOpen && (
        <div
          className="fixed left-0 z-30 flex items-end justify-center bg-black/70 p-4"
          style={{ top: 'var(--app-offset-top)', width: 'var(--app-width)', height: 'var(--app-height)' }}
