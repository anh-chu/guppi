...
| `go test -race ./pkg/commands/server -count=1` | **FIXED** | Was flaky (`TestRuntimeEnricherEnrichIsPureCacheLookup` vs. a leaked background goroutine from a *different* test, `TestRuntimeCancellationStops`, both racing the same package-level `readProcCwd` test-double variable). Fixed by giving `runtimeEnricher` an instance-level `readCwd` override field instead of monkeypatching the shared package var; the two affected tests now inject their fake via the struct field, so no test-only global remains for a leaked goroutine to race. `Runtime.Stop()` and its shutdown-wait guarantee were left untouched (no new `WaitGroup`/goroutine-exit tracking added). Confirmed clean across 10 consecutive `go test -race ./pkg/commands/server/... -count=2` runs. |
