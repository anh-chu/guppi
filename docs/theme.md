...
Existing themes: `raycast` (labeled "Default"), `dark`, `light`, plus a user-editable `custom` theme built at runtime from a persisted palette (see `buildCustomThemePreset` in `web/src/theme.ts`) rather than a static `themePresets` entry.
