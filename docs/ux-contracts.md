...
**Contract:** Single-line row: a **leading state dot** (colored by status via `statusBadgeConfig`; when a loud event is present the dot is warning-colored and pulses, so attention outranks the underlying status; the status word is exposed via `title`/`aria-label`), agent mark icon (if agent active), session name, optional AI-naming spinner, and uptime (e.g. "45m"). The worktree parent + branch glyph still prefixes worktree sessions. The **`project/` name prefix is shown only when the row is not inside a project group** (i.e. in status view, the Hidden list, and multi-host host groups); inside a project group the header already names the project. There is no second activity line and no separate status badge (the removed activity line previously showed live tool/prompt fallback text). In collapsed mode, shows only the first uppercase label character centered in the 44px rail, with an optional host dot at the row's top-right corner; no avatar circle, no status ring, no tooltip. When a session has more than one pane, an expandable pane list still renders beneath the row. Drag to reorder: visual drop indicators (edge zones → reorder, middle → pair/split). Right-click → context menu. Selection highlights row. Enter/Space confirm selection. Inline rename on double-click or rename menu item (Enter/Escape to submit/cancel).

Expanded row padding is compact (`px-2 py-1.5`) to suit the single line. Tiled-group members render via the same row (with `⊞ N` tile flag on the primary and an origin chip on cross-project companions); the group name + AI-name/rename affordance is preserved as the group-meta line above the primary.

**Why it matters:** The single-line, name-first, compact row with a state-carrying dot is the core scannability contract; re-adding the activity line or badge, restoring the taller padding, or dropping the attention-color override would change density and the at-a-glance status signal. The tiled group-meta line keeps group AI-name/rename reachable in project view. Drag/right-click/rename interaction modes remain user-facing contracts.
