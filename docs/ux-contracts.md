...
**Contract:** Two modes, persisted to `termyard:view-mode` (values `'project'` | `'status'`; any legacy/absent value resolves to `'project'`). **Project view (default):** on a single host, sessions and tiled/layout groups are bucketed by `project_path` under collapsible project headers that reuse the host-header pill element (filled `bg-white/[0.04]` pill, chevron + `font-medium` project label + a state dot [warning if any member needs attention, else green if any working, else mute] + `· N` count). The project label shows the trailing two path segments (`parent/leaf`, home-collapsed) so generic leaves like `app`/`web` stay distinguishable. A tiled group renders as: its **primary** (active-key, else first-leaf) session as a normal row flagged with a `⊞ N` tile count and carrying the group's **hover AI-name / rename buttons**, then its **companions indented under a left connector**, each showing an origin chip when their own project differs from the primary's. A faint group-name meta line appears above the primary only when the group is named or being renamed (the rename input renders there); unnamed groups show no meta line. Project header collapse state persists to `termyard:collapsed-projects`. When multiple hosts are connected, project view falls back to the existing host-grouped rendering unchanged; the 44px collapsed rail always renders the flat list. **Status view:** five status buckets (Needs attention, Working, Idle, Shell, Process). Toggle via the toolbar "Group by status" button (flips project ↔ status). A persistent search field above the toolbar filters `visibleSessions` by name/label/project (Esc or × clears).
