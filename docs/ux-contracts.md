...
Expanded row padding is compact (`px-2 py-1.5`) to suit the single line. Tiled-group members render via the same row (with a fill-less `⊞ N` tile flag on the primary and a borderless origin chip on cross-project companions). The redundant per-row host-color dot was removed from expanded rows (host is carried by the per-host section + header chip); it remains only in the 44px collapsed rail. The group name is not shown in project view; AI-name/rename are hover buttons on the primary row, and a rename input appears above the primary only while that group is actively being renamed.

**Why it matters:** The single-line, name-first, compact row with a state-carrying dot is the core scannability contract; re-adding the activity line or badge, restoring the taller padding, or dropping the attention-color override would change density and the at-a-glance status signal. Group AI-name/rename stay reachable as hover buttons on the primary row. Drag/right-click/rename interaction modes remain user-facing contracts.
