...
**Contract:** Underlined file paths (e.g., `/home/user/file.rs:12:3`, `./relative/path.js:1`) clickable in terminal. Click → open in wiki panel (or fallback new-tab artifact-viewer). Hover → pointer cursor + underline. Heuristic: path with extension and optional line/column suffix (`:N:M`). URLs (http://, etc.) handled separately by WebLinksAddon. Relative and bare paths resolve against the session's active pane cwd; `~` / `~/...` resolves against the server's home dir. Before a path is highlighted, the client checks it against `GET /file/exists` (cached, fails open on error/remote-host) — paths confirmed not to exist are not underlined or clickable.

**Why it matters:** File link recognition heuristic is a contract; changing it would affect which paths are clickable. The existence check is a contract too: it must fail open (never hide a real link on a network hiccup) and must not mint a file-open capability just to check.

**Verification pointer:** `web/src/lib/fileLinkProvider.ts`, `web/src/lib/pathExists.ts`, `pkg/server/routes_files.go` (`resolveFilePath`, `handleFileExists`)
