...
**Contract:** Session list grouped by project (cwd) by default, or by status, plus custom layout groups (see 2.1). Sidebar can be collapsed to narrow column or completely hidden; width adjustable via drag handle. Per-group collapse toggles. Drag-drop sessions to reorder. Click selects session. Right-click → context menu. Toolbar: persistent search field, then filter by project, toggle status-view grouping, quick-shell button, collapse toggle.
...
**Contract:** Two modes, persisted to `termyard:view-mode` (values `'project'` | `'status'`; any legacy/absent value resolves to `'project'`). **Project view (default):** sessions and tiled/layout groups are bucketed into **one section per host+path** (a tiled group inherits its primary/first-leaf session's host and path) under collapsible **plain-text** headers (no fill/border: chevron + muted `font-medium` project label that brightens to ink on hover + optional borderless host chip + a warning **dot (pulsing)** only when a member needs attention; **working** state is signaled instead by a **shimmer sweep on the header label** (`.text-shimmer`, disabled under `prefers-reduced-motion`); idle sections show neither + muted count). Consecutive sections are separated by a faint full-width **hairline + top gap** (`mt-2 border-t border-hairline`, skipped on the first section) for skimmability. **Design rule:** backgrounds/borders are reserved for interaction state (selection, hover, inputs) plus this single group-separator hairline; structure is otherwise carried by whitespace + weight, so headers, the agent glyph, the `⊞ N` tile flag, and the host/origin chips are all fill-less and border-less. The project label shows the trailing two path segments (`parent/leaf`, home-collapsed) so generic leaves like `app`/`web` stay distinguishable. **Host is split, not merged:** the same path on two hosts is two separate sections keyed by `hostId\0path`. A **host chip** (host name; `· offline` appended and the whole section dimmed when the host is offline) appears on the header only for **remote** hosts (`section.hostId !== localHostId`); local (“This machine”) sections show no chip, to avoid repeating it on every row. There is no separate host-grouped view. A tiled group renders as: its **primary** (active-key, else first-leaf) session as a normal row flagged with a `⊞ N` tile count, then its **companions indented under a left connector**, each showing an origin chip when their own project differs from the primary's. **Group naming has been removed** (no AI-name/rename UI, no server auto-namer): a group is identified by its primary row and members; group labels in other views show the joined member names. Project header collapse state persists to `termyard:collapsed-projects`, keyed by the host+path section key. **Reordering:** rows drag to reorder within their section (updates `sessionOrderRanks`); section headers drag to reorder whole cwd groups (a green insertion line shows above/below the target header), persisted as an ordered list of section keys in `termyard:project-order` (sections never moved keep their first-seen order and sort after moved ones). The 44px collapsed rail always renders the flat list. **Status view:** five status buckets (Needs attention, Working, Idle, Shell, Process). **Toolbar (single row):** left-to-right, a **search field** filling most of the row (filters `visibleSessions` by name/label/project; Esc or × clears), then a **project-filter icon button** (funnel; accent-tinted with a count badge when filters are active; opens the project multiselect popover), the **group-by-status** toggle (flips project ↔ status), and the **sidebar-collapse** button. **New session per workspace:** each workspace heading has a hover-revealed **`+`** that creates a session in that heading's cwd (`onQuickShell(section.path)` → optimistic session with that `project_path`); the global Quick Shell button was removed in favor of this. When there are **no sessions at all**, a single **“home”** heading is still shown so its `+` can start the first session (in the default/home cwd).

**Why it matters:** Project grouping is the primary navigation model; scattering a project's sessions across status buckets or dropping the per-project collapse/persist would break how users find related work. Status view remains an alternative lens.
...
**Contract:** Single-line row: a **leading dot** shown **only for attention** (warning, pulsing) or **just-done** (green flash) — no reserved slot, so rows without a dot sit flush at the glyph; **working** rows instead shimmer the session name (`.text-shimmer`), and idle/other rows show no dot; the status word is still exposed via `title`/`aria-label`. Then a **bare** agent mark glyph (borderless, no box — `AgentMark bare`; shell glyph muted), session name, optional AI-naming spinner, and uptime (e.g. "45m"). The worktree parent + branch glyph still prefixes worktree sessions. The **`project/` name prefix is shown only when the row is not inside a project section** (i.e. in status view and the Hidden list); inside a project section the header already names the project. There is no second activity line and no separate status badge (the removed activity line previously showed live tool/prompt fallback text). In collapsed mode, shows only the first uppercase label character centered in the 44px rail, with an optional host dot at the row's top-right corner; no avatar circle, no status ring, no tooltip. When a session has more than one pane, an expandable pane list still renders beneath the row. Drag to reorder: visual drop indicators (edge zones → reorder, middle → pair/split). Right-click → context menu. Selection highlights row. Enter/Space confirm selection. Inline rename on double-click or rename menu item (Enter/Escape to submit/cancel).

Expanded row padding is compact (`px-2 py-1.5`) to suit the single line. Tiled-group members render via the same row (with a fill-less `⊞ N` tile flag on the primary and a borderless origin chip on cross-project companions). The redundant per-row host-color dot was removed from expanded rows (host is carried by the per-host section + header chip); it remains only in the 44px collapsed rail. Group naming is removed entirely: there are no AI-name/rename affordances and no group-name display; the group label is always the joined member names.

**Why it matters:** The single-line, name-first, compact row with a state-carrying dot is the core scannability contract; re-adding the activity line or badge, restoring the taller padding, or dropping the attention-color override would change density and the at-a-glance status signal. Drag/right-click/rename (session) interaction modes remain user-facing contracts.
...
**Contract:** Click Overview card → docks terminal in right split (resizable, width persists). Right-split header has "Open full view" button (opens full session view) and close button (kills split, session stays alive). Multiple open panes show split layout: horizontally stacked, vertically stacked, or grids. Drag dividers to resize (min 200px per pane). Each pane's chrome is the **shared TerminalHeader** (see 9.14), which provides Split H/V, Pop out (tree-aware: removes the leaf and floats), Detach/close, and Kill (two-step, auto-disarm on header leave); in tiled mode the header bar also doubles as the pane drag-handle (`application/x-termyard-pane`). Drag-drop sessions onto edges or center: edge = split in that direction, center = swap or split, sides = move pane. Full-screen toggle per pane via `$mod+Shift+F` (exits with Esc).

**Phone single-in-view:** On phones (`(pointer: coarse)` with short side <540px, matched in either orientation; excludes tablets and desktop), a multi-pane group does not tile. It shows one pane at a time with a horizontal tab strip at top (one tab per session, active tab highlighted) to switch the visible pane. All panes stay mounted (hidden inactive ones keep their terminal size via CSS `visibility`), so switching preserves scrollback and connection. Single-pane groups render normally. Tablets and desktop keep the full tiled split layout.

**Why it matters:** The resizable split mechanism, pop-out button, drag-drop zones, full-screen toggle, and the phone single-in-view collapse are all observable features.
...
**Contract:** A single shared **`TerminalHeader`** renders at the top of every pane (single and tiled — both mount `Terminal`), in two variants: a **bar** on desktop and an **overlay glassy pill** on coarse-pointer/mobile (idles at 60% opacity, full on pane hover/focus). Bar-only left side: breadcrumb `cwd-leaf / session-name`. Right controls (both variants, same set single and tiled): **Compose** ($mod+Shift+U), **artifact count badge** (toggles preview panel; only when files detected), **split-h**, **split-v** (only when an `onSplit` handler is passed), **pop-out**, **fullscreen** (only on the active pane), **close**, and **kill** (two-step confirm; auto-disarms after ~2500ms and on header mouse-leave). Context-dependent behavior: in **single** view, close returns to Overview and kill kills the session, and pop-out floats the terminal in place; in **tiled** view, close detaches the pane from the group and pop-out is tree-aware (removes the leaf and floats), and the bar doubles as the pane drag-handle (`draggable`, sets `application/x-termyard-pane`, guards button clicks). This **replaces** the former floating glassy pill and the separate per-tile header (deduped into one component). Disconnect overlay: pulsing red dot + "Disconnected — Reconnecting" when not connected (absolute inset-0 z-10, pointer-events-none, doesn't block input).
...
**Contract:** Group trees are server-authoritative; enforcement of exclusive membership and dissolution happens on the backend on every write path (SetTree, ApplyRemote, ApplySnapshot, MigrateKey). After any write, a session key is a leaf in at most one live group — the most-recently-written group wins, others' leaves are pruned, and groups falling below 2 leaves are tombstoned (marked deleted, timestamp updated, persisted to `groups.json` but not removed). Peer tree deltas with timestamps older than a tombstone timestamp are rejected by last-write-wins comparison, ensuring tombstones are never resurrected. Clients adopt the server's exclusive membership via `groups/snapshot` (authoritative) and never push partial divergent state back; only user-initiated edits (split/close/resize/drag/pair) trigger PUTs, not group switches or passive reconciliation. Dead sessions (owner host online, session absent from `/api/sessions`) are pruned opportunistically on GET `/api/groups`, healing corrupt group trees. (Group AI naming was removed entirely: the naming coordinator, its `GroupEnforcementCoordinator` peer hook, and `Manager.GenerateGroupName` are deleted; the low-level `name` field/op and `GroupStore.SetName` remain as vestigial protocol/persistence with no caller.)
...
| `/api/groups` | POST | `{id, op, tree?, name?, rank?}` | `{Live}` | op ∈ tree/name/rank/delete (`ai-name` removed; `name` vestigial) |
