...
**Contract:** Board is the only layout. Displays four columns (Needs you / Working / Idle / Offline) with session cards, plus three collapsible rails below (Scheduled, Hidden, Backgrounded). Columns auto-fit width proportionally; rails collapse/expand independently. No layout toggle.

**Why it matters:** The board's column organization and rail structure are the primary view; their persistence is essential for users to organize sessions by state.
...
**Contract:** Stores docked terminal split width in pixels (`overview_split_width`, default 520px, clamped 360–1200px) and per-rail collapse state (`overview_rail_hidden`, `overview_rail_bg`, `overview_rail_scheduled` = "open"/"closed").
...
**Contract:** Opened via `$mod+,` (Ctrl/Cmd+Comma). Drawer with 4 buckets: Look (themes, terminal font/size/scrollback/renderer/graphemes/echo, fullscreen alerts toggle), Yard (default view, sidebar collapse mode, AI naming endpoint/key/model), Alerts (push notifications toggle, alert statuses multi-select, auto-dismiss seconds, agent setup status), System (peer list + connect/forget/reconnect buttons, wiki install button, security auto-lock timeout, sign-out button, about + version/update rows). All changes apply instantly (no Save button); feedback "SAVING…" during PUT.
...
**Contract:** When `terminal.predictive_echo` pref enabled: as user types printable ASCII characters (0x20–0x7e only), a visual echo overlay appears at cursor in 0.4 opacity italic styling, showing predicted text. Cleared when: (a) authoritative PTY output arrives (replaces overlay), (b) control character typed (Backspace, Enter, etc.), (c) multi-byte/UTF-8 input, (d) 500ms with no output, (e) alt-screen active (vim/less/fzf suppress prediction). Never calls terminal.write, never alters bytes, never reads terminal content — overlay is thin decoration only.

**Why it matters:** Echo delay perception is user-facing; predictive echo is opt-in and must clear correctly under all edge cases.

**Verification pointer:** `web/src/lib/terminal/predictive-echo.ts`

### 9.7 File links in terminal

**Contract:** Underlined file paths (e.g., `/home/user/file.rs:12:3`, `./relative/path.js:1`) clickable in terminal. Click → open in wiki panel (or fallback new-tab artifact-viewer). Hover → pointer cursor + underline. Heuristic: path with extension and optional line/column suffix (`:N:M`). URLs (http://, etc.) handled separately by WebLinksAddon.

**Why it matters:** File link recognition heuristic is a contract; changing it would affect which paths are clickable.

**Verification pointer:** `web/src/lib/terminal/fileLinkProvider.ts`
...
**Contract:** Font family select (9 options: Space Mono [default], JetBrains Mono, Fira Code, Menlo, Monaco, Consolas, Courier New, Inconsolata, monospace). Font size 8–32px (default 13, clamped). Renderer: DOM or WebGL (toggle). Unicode graphemes toggle (xterm addon). All settings apply instantly, xterm re-renders. Terminal theme drives 21 ANSI colors + cursor + selection background.

**Why it matters:** Font and rendering choices are user preference contracts; removing options would limit accessibility.

**Verification pointer:** `web/src/components/Terminal.tsx`, Settings/Look section
...
| `/file/grant` | POST | query `path`, `session?`, `host?` | `{token, path, root?}` | Artifact download token |
...
**Note:** `peer-connected` / `peer-disconnected` are handled in code but are **likely dead paths to browser** (hub subscribes to state.Manager only, not peer.Manager). Frontend App.tsx still handles them; path status TBD.
...
[Showing lines 1-743 of 752 (50.0KB limit). Use offset=744 to continue.]